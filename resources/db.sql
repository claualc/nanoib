CREATE DATABASE if not exists nanoib;
...
create table if not exists balances(
  acc_branch_id int not null default 0,
...
create table if not exists statement_items(
  ...
  ctpt_acc_branch_id int not null default 0,
  ctpt_acc_id int not null default 0,
  foreign key (acc_branch_id, acc_id) references accounts(branch_id, id),
...
create procedure get_user(
  in p_acc_branch_id int,
  in p_acc_id int,
  out out_holders_name varchar(40),
  out out_pwd binary(32),
  out out_pwd_salt binary(8)
) reads sql data
begin
  select
    holders_name,
    pwd,
    pwd_salt
  into out_holders_name, out_pwd, out_pwd_salt
  from
    accounts
  where
...
create procedure get_balance(
  in p_acc_branch_id int,
  in p_acc_id int,
  out out_balance numeric(20,2),
  out out_op_count int
) reads sql data
begin
  select
    balance,
    op_count
  into out_balance, out_op_count
  from
    balances
  where
...
create procedure transfer(
  in p_orig_acc_branch_id int,
  in p_orig_acc_id int,
  in p_dest_acc_branch_id int,
  in p_dest_acc_id int,
  in p_value numeric(20,2),
  in p_comment text
)
begin
  declare v_orig_acc_balance numeric(20,2);
  declare v_orig_acc_opcount int;
  declare v_dest_acc_opcount int;
...
create procedure count_statement_items(
  in p_acc_branch_id int,
  in p_acc_id int,
  out p_count int
)
...
create procedure get_statement_items(
  in p_acc_branch_id int,
  in p_acc_id int,
  in p_offset int,
  in p_result_max_size int,
  out t1_op_count int,
  out t1_tx_timestamp timestamp,
  out t1_in_out tinyint,
  out t1_value float,
  out t1_comment text,
  out t1_ctpt_acc_branch_id int,
  out t1_ctpt_acc_id int,
  out t2_holders_name text
) reads sql data
begin
  select
    t1.op_count,
    ...
    t1.ctpt_acc_id,
    t2.holders_name
  into t1_op_count, t1_tx_timestamp, t1_in_out, t1_value, t1_comment, t1_ctpt_acc_branch_id, t1_ctpt_acc_id, t2_holders_name
  from
